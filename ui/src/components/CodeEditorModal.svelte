<script lang="ts">
  ...
  /**
   * Which grammar to highlight with. The editor was built for the code
   * node and hardcoded to Python; a SQL query deserves the same window,
   * the same line numbers and the same highlighting rather than a
   * four-line textarea, and the only thing that differs is the grammar.
   */
  export let language: "python" | "sql" = "python";
  /** Overrides the window title, which otherwise names the language. */
  ...
  $: if (visible) {
    localScript = script || "";
    // language is read here so switching it re-highlights rather than
    // leaving the previous grammar's colours behind.
    void language;
    doHighlight();
    tick().then(() => {
      textareaEl?.focus();
  ...
  function doHighlight() {
    ...
    try {
      // Falling back to plain text rather than throwing: an unhighlighted
      // editor is a small loss, a blank one is a broken feature.
      highlighted = grammar ? Prism.highlight(code, grammar, language) + "\n" : code + "\n";
    } catch {
      highlighted = code + "\n";
    }
  }
    ...
    <div class="modal">
      <div class="modal-header">
        <div class="header-left">
          <span class="modal-title">{windowTitle}</span>
