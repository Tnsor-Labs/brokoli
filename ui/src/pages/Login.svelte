<script lang="ts">
  ...
  onMount(() => {
    ...
    // Only show OAuth buttons for providers this deployment actually has configured
    // (core has none of its own — this is set by enterprise's platform provider).
    fetch("/api/auth/methods")
      .then((res) => (res.ok ? res.json() : { oauth: [] }))
      .then((data) => {
        oauthProviders = data.oauth || [];
      })
      .catch(() => {
        oauthProviders = [];
      });
      ...
      {#if oauthProviders.length > 0}
        <div class="oauth-buttons">
          {#if oauthProviders.includes("github")}
            <a
              href="/api/auth/oauth/github?redirect_uri={encodeURIComponent(
          ...
          {#if oauthProviders.includes("google")}
            <a
              href="/api/auth/oauth/google?redirect_uri={encodeURIComponent(
                window.location.origin,
              )}"
              class="oauth-btn"
            >
          ...
          {/if}
          {#if oauthProviders.includes("keycloak")}
            <a
...
<style>
  .login-page {
    ...
    background:
      radial-gradient(ellipse 900px 500px at 50% 0%, rgba(255, 255, 255, 0.05), transparent 70%),
      var(--bg-primary);
    padding: var(--space-xl) var(--space-md);
  }
  :global([data-theme="light"]) .login-page {
    background:
      radial-gradient(ellipse 900px 500px at 50% 0%, rgba(0, 0, 0, 0.03), transparent 70%),
      var(--bg-primary);
  }
