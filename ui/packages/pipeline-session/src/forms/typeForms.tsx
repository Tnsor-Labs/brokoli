    ...
    <>
      ...
      {!conn && (
        <TextField
          ...
          required
          hint="Stored in the pipeline as written. Prefer a saved connection for credentials."
        />
      )}
    ...
    <>
      <Section title="Request">
        <ConnectionField
          ctx={ctx}
          types={HTTP_TYPES}
          noneLabel="No connection (full URL below)"
          hint="A connection supplies the base URL, headers and credentials."
        />
        <TextField
          ctx={ctx}
          name="url"
          ...
          required
          templatable
          placeholder={ctx.get('conn_id') ? '/v1/orders' : 'https://api.example.com/v1/orders'}
          hint={
            ctx.get('conn_id')
              ? "A path starting with / is appended to the connection's base URL."
              : undefined
          }
        />
        <SelectField
        ...
        {['POST', 'PUT', 'PATCH'].includes(method) && (
          <TextField
            ...
            rows={5}
            templatable
            placeholder='{"since": "${interval.start}"}'
            hint="Sent as written. Set a Content-Type header if the API needs one."
          />
        )}
      ...
      <Section title="Response">
        ...
        {response === 'dataset' && (
          <TextField
            ...
            hint="Dot path to the array of records. Leave empty when the body is the array."
          />
        )}
        {response === 'scalar' && (
          <TextField ctx={ctx} name="value_path" label="Value path" mono placeholder="meta.total" />
        )}
        <LegacyKeyNotice
          ctx={ctx}
          name="response_path"
          fix={{
            label: 'Move it to Records path',
            patch: { records: legacy, response_path: undefined },
          }}
        >
          The engine reads the record location from "Records path". The old setting "{legacy}" is
          ignored.
        </LegacyKeyNotice>
      </Section>
    </>
      ...
      <Section title="Script">
        ...
        {bundle ? (
          <Callout tone="info" title="Runs a task bundle">
            This node runs the packaged bundle{' '}
            {bundle.digest ? <code>{bundle.digest.slice(0, 19)}</code> : ''} published by an SDK. A
            script cannot be added alongside it.
          </Callout>
        ) : (
          <ScriptField
            ctx={ctx}
        ...
        {language === 'typescript' ? (
          <TextField
            ...
            mono
            placeholder="Detected automatically (Node 20 or newer)"
          />
        ) : (
          <TextField
            ctx={ctx}
...
function Join({ ctx, nodes, edges }: TypeFormProps) {
  const inputEdges = edges.filter((e) => e.to === ctx.node.id)
  const inputs = inputEdges.map((e) => nodes.find((n) => n.id === e.from)?.name ?? e.from)
  const leftSchema = inputEdges[0]
    ? outputSchemaForNode(inputEdges[0].from, nodes, edges)
    : undefined
  const rightSchema = inputEdges[1]
    ? outputSchemaForNode(inputEdges[1].from, nodes, edges)
    : undefined
  const preview = joinOutputSchema(leftSchema, rightSchema, ctx.node.config ?? {})
  const leftKey = str(ctx.get('left_key'))
  const rightKey = str(ctx.get('right_key')) || leftKey
  const keyError = (schema: DatasetSchema | undefined, key: string, side: string) =>
    schema && key && !schema.columns.some((column) => column.name === key)
      ? `${side} key is not in the declared columns.`
      : undefined
  return (
    <>
      <Section
        ...
        description={
          inputs.length === 2
            ? `Left: ${inputs[0]}. Right: ${inputs[1]}. The first connection made is the left side.`
            : 'Connect exactly two inputs. The first connection made is the left side.'
        }
      >
        <SelectField
          ctx={ctx}
        ...
        <SchemaKeyField
          ...
          schema={leftSchema}
          error={keyError(leftSchema, leftKey, 'Left')}
        ...
        <SchemaKeyField
          ...
          schema={rightSchema}
          error={keyError(rightSchema, rightKey, 'Right')}
          placeholder="Same as the left key"
          hint="Right-side columns with a clashing name get a right_ prefix."
        />
        <SelectField
          ctx={ctx}
          name="collision_policy"
          label="Name collisions"
          defaultLabel="prefix right-side columns"
          options={[
            { value: 'error', label: 'Reject collisions' },
            { value: 'prefix', label: 'Prefix with right_' },
            { value: 'alias', label: 'Prefix with an alias' },
          ]}
        />
        {str(ctx.get('collision_policy')) === 'alias' && (
          <TextField
            ctx={ctx}
            name="right_alias"
            label="Right-side alias"
            mono
            required
            placeholder="right_customer"
          />
        )}
      </Section>
      <Section
        title="Output schema"
        description="Derived from declared upstream schemas. Runtime-inferred columns appear after the join runs."
      >
        {preview.error ? (
          <Callout tone="danger" title="Output schema cannot be derived">
            {preview.error}
          </Callout>
        ) : preview.columns ? (
          <SchemaColumns columns={preview.columns} />
        ) : (
          <p className="ps-form-note">
            Declare schemas on both inputs to preview the join output here.
          </p>
        )}
      </Section>
    </>
...
function SchemaKeyField({
  ...
  const list = `${ctx.node.id}-${name}-columns`
  return (
    <>
      <Field
        label={label}
        hint={hint}
        error={error || (required && !value.trim() ? 'Required' : undefined)}
        required={required}
      >
        <Input
          value={value}
          ...
          onChange={(event) =>
            ctx.set({ [name]: event.target.value }, `field:${ctx.node.id}:${name}`)
          }
        />
      </Field>
      {schema && (
...
function SchemaColumns({ columns }: { columns: { name: string; type?: { kind?: string } }[] }) {
  return (
    <dl className="ps-schema-columns">
      {columns.map((column) => (
        <div key={column.name}>
          <dt>
            <code>{column.name}</code>
          </dt>
          <dd>{column.type?.kind ?? 'unknown'}</dd>
        </div>
      ))}
    </dl>
  )
...
function SqlGenerate({ ctx }: TypeFormProps) {
  return (
    <Section
      title="SQL output"
      description="Produces one row with a sql_output column; connect it to a File Output (SQL) or a Database Sink."
    >
  ...
  return (
    <Section title="File">
      ...
      <SelectField
        ...
        defaultLabel={`from the extension (${inferred})`}
      />
      {path.toLowerCase().endsWith('.tsv') && (
        <p className="ps-form-warning">.tsv files are written comma-separated.</p>
      )}
      {effective === 'sql' && (
        <>
          <TextField
    ...
    <>
      <Section title="Destination">
        ...
        <TextField
          ...
          hint={
            fromSqlGenerate
              ? 'Optional: the upstream SQL Generate node supplies the statements.'
              : 'schema.table is allowed.'
          }
        />
      </Section>
      <Section title="Write behaviour">
        ...
        {mode === 'overwrite' && (
          <CheckField
            ...
            name="truncate"
            label="Use TRUNCATE"
            description="Faster than DELETE on large tables."
          />
        )}
        ...
        <TextField
          ctx={ctx}
          name="table_engine"
          label="Table engine (ClickHouse)"
          mono
          placeholder="MergeTree ORDER BY id"
        />
      </Section>
    </>
  )
...
function SinkApi({ ctx }: TypeFormProps) {
  return (
    <Section
      title="Request"
      description="Rows are sent as JSON arrays, one request per batch. Any HTTP status of 400 or above fails the node."
    >
      ...
      <MapField
        ...
        addLabel="Add a header"
        keyPlaceholder="Header-Name"
        hint="Content-Type: application/json is always sent."
      />
    </Section>
  )
}
  ...
  return (
    <>
      <Section title="Source">
        <ConnectionField
          ctx={ctx}
          idKey="source_conn_id"
          uriKey="source_uri"
          types={DB_TYPES}
          label="Source connection"
          noneLabel="Enter a source URI instead"
        />
        {!ctx.get('source_conn_id') && (
          <TextField ctx={ctx} name="source_uri" label="Source URI" mono required />
        )}
      ...
      <Section title="Destination">
        ...
        />
        {!ctx.get('dest_conn_id') && (
          <TextField ctx={ctx} name="dest_uri" label="Destination URI" mono required />
        )}
        <TextField
          ctx={ctx}
          name="dest_table"
        ...
        <SelectField
          ...
          label="Dialect"
          options={DIALECTS}
          defaultLabel="inferred from the destination"
        />
        <SelectField
          ctx={ctx}
          name="mode"
        ...
        />
        {mode === 'upsert' && (
          <ListField ctx={ctx} name="key_columns" label="Key columns" required />
        )}
        <NumberField
          ctx={ctx}
          name="chunk_size"
...
function Condition({ ctx }: TypeFormProps) {
  ...
  return (
    <Section
      title="Condition"
      description="Connections leaving this node are marked true or false. Downstream nodes on the branch that does not match are skipped."
    >
      <Field
        label="Expression"
        error={
          supported
            ? undefined
            : 'Not in the supported grammar. Under IR 2.1 the server rejects it; under IR 2.0 it silently evaluates to false.'
        }
        hint={
          !expr
            ? 'Empty passes everything through as true.'
            : supported
      ...
      >
        <Input
          ...
          onChange={(e) =>
            ctx.set({ expression: e.target.value }, `field:${ctx.node.id}:expression`)
          }
        />
      </Field>
    ...
    <>
      <Section title="Command">
        <SelectField
          ...
          name="command"
          label="Command"
          options={['run', 'test', 'build', 'seed', 'snapshot', 'compile', 'ls']}
          defaultLabel="run"
        />
        <TextField
          ctx={ctx}
          ...
          hint="dbt selection syntax, passed as --select."
        />
      ...
      <Section title="Warehouse">
        <ConnectionField
          ctx={ctx}
          types={DBT_TYPES}
          noneLabel="Use the project's own profiles.yml"
          hint="With a connection, Brokoli writes the dbt profile for you."
        />
        {conn ? (
          <>
            ...
            <TextField
              ...
              hint="Hands this model's table to downstream nodes."
            />
          </>
        ) : (
          <TextField
            ...
            mono
            placeholder="The profile's default"
          />
        )}
        {conn && ctx.get('profiles_dir') !== undefined && (
          <p className="ps-form-warning">
            A profiles directory is set; it takes precedence over the connection.
          </p>
        )}
      </Section>
      <Section title="Advanced">
        ...
        <TextField
          ...
          hint="YAML or JSON, passed as --vars."
        />
      </Section>
    </>
  )
  ...
  return (
    <Section title="Notification">
      ...
      <TextField
        ctx={ctx}
        name="webhook_url"
        label={slack ? 'Slack webhook URL' : 'Webhook URL'}
        mono
        required
      />
      {slack && (
        <TextField ctx={ctx} name="channel" label="Channel" mono placeholder="#data-alerts" />
      )}
      <TextField
        ctx={ctx}
        name="message"
        ...
        multiline
        rows={3}
        placeholder="Pipeline {{pipeline}} completed"
        hint="{{pipeline}}, {{run_id}} and {{rows}} are filled in."
      />
    </Section>
  )
}
...
function Wait({ ctx }: TypeFormProps) {
  ...
  return (
    <Section
      title="Wait for"
      description="The run is parked, without holding a run slot, until the condition is met or the timeout passes."
    >
      <SelectField
        ctx={ctx}
        name="condition"
      ...
      {condition === 'file_exists' && (
        <TextField ctx={ctx} name="path" label="Path" mono required />
      )}
      {condition === 'http' && (
        <>
      ...
      {condition === 'interval_elapsed' && (
        <TextField ctx={ctx} name="offset" label="Extra delay" mono placeholder="15m" />
      )}
      {condition === 'pipeline' && (
        <TextField ctx={ctx} name="pipeline_id" label="Pipeline id" mono required />
      )}
      <TextField
        ctx={ctx}
        name="poll_interval"
        ...
        mono
        placeholder="30s"
        hint="A duration such as 30s, 5m or 1h."
      />
      <TextField
        ctx={ctx}
        name="timeout"
        ...
        mono
        placeholder="6h"
        hint="A duration. This is the wait timeout, not a run attempt timeout."
      />
    </Section>
  )
}
...
function Union({ ctx }: TypeFormProps) {
  return (
    <Section title="Union">
      <p className="ps-form-note">
        Concatenates every connected input into one dataset. Connect at least two inputs, or one
        from a fan-out node.
      </p>
      {str(ctx.get('mode')) && str(ctx.get('mode')) !== 'union' && (
        <p className="ps-form-warning">Unsupported mode "{str(ctx.get('mode'))}".</p>
      )}
    </Section>
  )
}
...
function Task({ ctx }: TypeFormProps) {
  ...
  return (
    <>
      <Section
        title="Task bundle"
        description="Task nodes are authored and published with the Python or TypeScript SDK."
      >
        {bundle?.digest ? (
        ...
        {ctx.node.interface && (
          <pre className="ps-json-preview">{JSON.stringify(ctx.node.interface, null, 2)}</pre>
        )}
      </Section>
      <Section title="Limits">
        <NumberField ctx={ctx} name="timeout" label="Timeout (seconds)" defaultValue={30} min={1} />
...
function DatasetFunction({ ctx }: TypeFormProps) {
  ...
  const setFn = (patch: Record<string, unknown>, key?: string) =>
    ctx.set(
      {
        function: Object.fromEntries(
          Object.entries({ ...fn, ...patch }).filter(([, v]) => v !== undefined && v !== ''),
        ),
      },
      key,
    )
    ...
    <Section title="Function">
      <Field label="Function name" required error={!fn.name ? 'Required' : undefined}>
        <Input
          mono
          value={fn.name ?? ''}
          onChange={(e) => setFn({ name: e.target.value }, `field:${ctx.node.id}:fn`)}
        />
      </Field>
      <ScriptField
        ctx={{
          ...ctx,
          get: (k) => (k === '__fn_script' ? fn.script : ctx.get(k)),
          set: (patch) => setFn({ script: patch.__fn_script }),
        }}
        name="__fn_script"
