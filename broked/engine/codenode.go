...
func ExecuteCodeNode(script string, input *common.DataSet, nodeConfig map[string]interface{}, runParams map[string]string, timeoutSec int) (*common.DataSet, string, error) {
	...
	if useFileMode {
		...
		inputNDJSON = filepath.Join(tmpDir, fmt.Sprintf("broked_in_%d.ndjson", time.Now().UnixNano()))
		outputNDJSON = filepath.Join(tmpDir, fmt.Sprintf("broked_out_%d.ndjson", time.Now().UnixNano()))
		...
		if err := WriteArrowJSON(inputNDJSON, input); err != nil {
			...
			inputCSV = filepath.Join(tmpDir, fmt.Sprintf("broked_in_%d.csv", time.Now().UnixNano()))
			outputCSV = filepath.Join(tmpDir, fmt.Sprintf("broked_out_%d.csv", time.Now().UnixNano()))
