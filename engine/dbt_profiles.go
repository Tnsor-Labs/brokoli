...
var dbtAdapters = map[models.ConnectionType]dbtAdapter{
	...
	// ADR-025's read-back rule is satisfied since ADR-027 phase 1: a model
	// dbt builds on ClickHouse is readable by source_db and by the dbt
	// node's own output_model reference. driver: native because the
	// connection record's port is the native protocol's -- the same server
	// and port Brokoli itself dials -- so one connection means one address.
	models.ConnTypeClickHouse: {Type: "clickhouse", Distribution: "dbt-clickhouse", DefaultPort: 9000},
}
...
func generateDBTProfile(conn *models.Connection, profileName, targetSchema string, threads int) (*dbtProfile, error) {
	...
	if conn.Schema == "" && adapter.Type != "clickhouse" {
		// ClickHouse has no database/schema split: the profile's schema IS
		// the database dbt builds in (targetSchema, checked above), and
		// the connection's own database plays no part.
	...
	// The credential stanza differs per adapter. Postgres and MySQL take
	// dbname + schema; ClickHouse has no such split -- its schema IS the
	// database -- and needs the driver named, because dbt-clickhouse
	// defaults to HTTP on a port this connection does not describe.
	var stanza string
	switch adapter.Type {
	case "clickhouse":
		stanza = fmt.Sprintf(`      type: clickhouse
      driver: native
      host: %s
      port: %d
      user: %s
...
`,
			...
			yamlString(targetSchema), threads)
	default:
		stanza = fmt.Sprintf(`      type: %s
      host: %s
      port: %d
      user: %s
      password: %s
...
%s:
  ...
  outputs:
    %s:
%s`,
		yamlKey(profileName), target, target, stanza)
