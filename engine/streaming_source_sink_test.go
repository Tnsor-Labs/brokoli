...
func TestSinkCanStreamScope(t *testing.T) {
	...
	yes := []models.Node{
		...
		// A keyed upsert stages through the bulk writer (#377), so it
		...
		// ClickHouse appends stream since ADR-027 phase 2, overwrites
		// since phase 3.
	...
	no := []models.Node{
		...
		// ClickHouse upsert does not stream: refused for good.
