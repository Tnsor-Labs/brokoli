...
## Update — 2026-08-16: M3 phase 1 (`plan` command, protocol layer) shipped

The `plan`/`discover` relationship this ADR's Negative consequences flagged as unresolved is now settled: `plan` is an optional, per-stream refinement step after `discover`, not a replacement for it. A node type opts in with `NodeTypeDecl.SupportsPlan` (`pkg/plugins/manifest.go`); `Validate` rejects it set on anything but `kind=source`. `discover` had no real callers to break at the time this was decided, so there was no cheaper moment to fix the relationship than now.

`CmdPlan`/`MsgWorkUnit` landed as an additive protocol pair (`pkg/plugins/protocol.go`), parsed by the subprocess runner and exposed through `Runner.Plan`. A `WorkUnit`'s `UnitID` is the plugin's own name for one piece of work; the host derives its own durable instance key from it rather than the plugin dictating that key directly — the same split `MsgState` already draws between what the plugin knows (the cursor value) and what the host owns (persisting and replaying it). A later `read` call for one planned unit goes through a new `Runner.ReadUnit`, kept as a separate method rather than a new parameter on `Read`, so every existing caller of `Read` is untouched by this change.

**Correction to this ADR's original Decision (item 1):** scheduling and tracking work units "the way `#31`'s `expansion_instances` table already does" is superseded. ADR-015 (logical and physical execution plans), which landed five days after this ADR, already generalizes that exact tracking model — `models.PhysicalPlan`/`PhysicalInstance` and `store.PhysicalPlanStore`/`PhysicalInstanceStore`, already implemented and wired into `engine/runner.go`. ADR-015's own Follow-ups said as much ("update ADR-013 when connector `plan` work units use this model"); that update is this paragraph. The engine-wiring slice of M3 will persist work units through that model, not `ExpansionInstance`.

### Known limitation: not wired to the engine yet

Nothing calls `Runner.Plan` outside of tests. `pkg/plugins/manager.go`'s dispatch still calls `Read` unconditionally for every source, whether or not its manifest sets `SupportsPlan`. `engine/planner.go`'s `planNode` has no case for a plan-capable plugin node yet, so such a node still projects as `WorkUnitSingle` in the physical plan explanation. That is the next slice: teaching the planner and `Manager.Execute` to call `Plan` and dispatch one `ReadUnit` per unit — sequentially, per the scope agreed on `#39` — persisting each as a `models.PhysicalInstance`.
