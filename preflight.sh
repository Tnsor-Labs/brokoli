...
#
#   ./preflight.sh                 full run (~13-19 min; long pole is
#                                   `go test -race ./...` — CI's own
#                                   comment on that step measured 581s on
#                                   a loaded machine, #329)
...
#   PREFLIGHT_SCAN_PROCS=N ./preflight.sh  CPU cap for gosec/govulncheck/
#                                   go-licenses while they run in the
#                                   background alongside the test suite
#                                   (default 2 — see the note above the
#                                   gosec docker run below for why this
#                                   exists)
#
...
#
# CPU note: unlike CI, which runs lint/test/build/security as five
# separate jobs on five separate VMs (zero contention by construction),
# every stage here shares one machine's cores. `go test -race ./...` is
# deliberately left at its default (uncapped) parallelism, matching
# ci.yml's own invocation exactly — it is the long pole and the
# correctness-critical step. The background gosec/govulncheck/go-licenses
# scans are capped via PREFLIGHT_SCAN_PROCS so they can make progress
# concurrently with the test suite (preserving the wall-clock win of not
# running them sequentially) without fighting it for every core.
set -euo pipefail
cd "$(dirname "$0")"
...
FAILED=0
START=$(date +%s)
PREFLIGHT_SCAN_PROCS="${PREFLIGHT_SCAN_PROCS:-2}"
...
(
  if command -v docker >/dev/null 2>&1; then
    # gosec's own image/digest is pinned deliberately (see below) and NOT
    # what makes this slow. `--rm` gives every invocation a brand-new
    # container with an empty $GOPATH/pkg/mod and empty build cache, so
    # without the two named volumes below, gosec re-downloads and
    # re-compiles this module's ENTIRE dependency graph from scratch on
    # every single preflight run, fighting go test -race for CPU the
    # whole time it does. Named volumes persist across --rm (only the
    # container is ephemeral, not the volume), so the first run after
    # this change is still cold; every run after that reuses them. This
    # is a separate, container-local cache -- never the host's own
    # GOMODCACHE/GOCACHE -- specifically to avoid a root-in-container
    # write leaving host-owned cache files an unprivileged host `go`
    # invocation can't later read or overwrite.
    docker run --rm -v "$PWD":/src -w /src \
      -v brokoli-preflight-gosec-gomodcache:/go/pkg/mod \
      -v brokoli-preflight-gosec-gocache:/gocache \
      -e GOCACHE=/gocache \
...
# Pinned exact versions, not @latest: a tool version bump on any given day
# would otherwise force a fresh, uncached compile (defeating the point of
# a warm build cache) and make results non-deterministic run to run.
# `go install` once into the host's own module/build cache (already 2.9G/
# 2.7G warm on a machine that's built this module before) rather than
# `go run` on every invocation -- `go run` would keep re-resolving the
...
# with GOBIN customized.
GOBIN_DIR=$(go env GOBIN)
[ -n "$GOBIN_DIR" ] || GOBIN_DIR="$(go env GOPATH)/bin"
(
  GOTOOLCHAIN="${MOD_TOOLCHAIN:-auto}" go install golang.org/x/vuln/cmd/govulncheck@$GOVULNCHECK_VERSION
  GOMAXPROCS="$PREFLIGHT_SCAN_PROCS" "$GOBIN_DIR/govulncheck" ./...
) > "$LOGDIR/govulncheck.log" 2>&1 &
VULN_PID=$!
...
(
  ...
  if [ -n "$CHANGED_UI" ]; then
    ...
    echo "prettier: $(echo "$CHANGED_UI" | wc -l) changed file(s) clean" >> "../$LOGDIR/ui-checks.verdict"
  else
    echo "prettier: no changed UI files" >> "../$LOGDIR/ui-checks.verdict"
  fi
) > "$LOGDIR/ui-checks.log" 2>&1 &
UI_CHECK_PID=$!
...
say "waiting on parallel security scans + UI checks"
...
wait "$UI_CHECK_PID" || fail ui-checks "$LOGDIR/ui-checks.log"
cat "$LOGDIR/ui-checks.verdict" 2>/dev/null || true
