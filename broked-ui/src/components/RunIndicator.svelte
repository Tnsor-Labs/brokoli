<script lang="ts">
  ...
  import { liveRunStatuses, events } from "../lib/stores";
  ...
  // Load recent runs on mount so indicator shows even without WS events
  onMount(async () => {
    try {
      const res = await fetch("/api/dashboard", { headers: authHeaders() });
      if (res.ok) {
        const data = await res.json();
        for (const r of (data.recent_runs || []).slice(0, 10)) {
          liveRunStatuses.update(s => ({ ...s, [r.run_id]: r.status as RunStatus }));
          recentRuns.push({ id: r.run_id, pipeline_id: r.pipeline_id, status: r.status, started_at: r.started_at });
        }
  ...
  // Derive active runs from liveRunStatuses
  $: runEntries = Object.entries($liveRunStatuses);
  $: runningCount = runEntries.filter(([, s]) => s === "running").length;
  $: hasActivity = runEntries.length > 0;

  // Flash color on completion/failure
  $: {
  ...
  {#if expanded}
    <div class="indicator-panel">
      ...
      <div class="panel-list">
        {#each runEntries as [runId, status]}
          {@const recent = recentRuns.find(r => r.id === runId)}
          {@const event = $events.find(e => e.run_id === runId)}
          {@const pipelineId = recent?.pipeline_id || event?.pipeline_id || ""}
          <a href="#/pipelines/{pipelineId}/runs" class="panel-item" on:click={() => expanded = false}>
            <span class="item-dot" class:dot-pulse={status === "running"} style="background: {statusColor(status)}"></span>
            <span class="item-name">{pipelineId ? pipelineId.slice(0, 8) : runId.slice(0, 8)}</span>
            <span class="item-status" style="color: {statusColor(status)}">{statusLabel(status)}</span>
            <span class="item-time">{recent?.started_at ? timeAgo(recent.started_at) : (event?.timestamp ? timeAgo(event.timestamp) : "")}</span>
          </a>
        {/each}
